...
import fetchContact from "../Advertiser_Components/Fetch/fetchContact";
...
const ContactEdit = (props) => {
  ...
  const result = useQuery(["contact", id_arda.id, token], fetchContact);
  if (result.isLoading) {
    return (
      <MDBSpinner
  ...
  const pushEditUser = async () => {
    axios
      .put(
        `https://tata-backend.onrender.com/api/updateContact/${id_arda.id}`,
        newData,
        {
          headers,
        }

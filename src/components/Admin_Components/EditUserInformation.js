...
import fetchUser from "../Advertiser_Components/Fetch/fetchUser";
import { useState } from "react";
import axios from "axios";
import ContactEdit from "./ContactEdit";
import MediaEdit from "./MediaEdit";
const EditUserInformation = (props) => {
  var id_arda = props.arda;
  ...
  const result = useQuery(["user1212", id_arda.id, token], fetchUser);
  if (result.isLoading) {
    return (
  ...
  const pushEditUser = async () => {
    axios
      .put(`https://tata-backend.onrender.com/api/updateUser/${id_arda.id}`, newData, {
        headers,
      })
      .then((response) => {
      ...
      <MDBRow>
        ...
        <MDBCol md="10" id="userInfo">
          ...
          <MDBCard id="userContact" style={{ marginTop: "25px" }}>
            ...
            <ContactEdit id_arda={id_arda}></ContactEdit>
          </MDBCard>
          <MDBCard id="userMedia" style={{ marginTop: "25px" }}>
            <MDBCardHeader style={{ fontWeight: "bold", fontSize: "1.2rem" }}>
              Media Links
            </MDBCardHeader>
            <MediaEdit id_arda={id_arda}></MediaEdit>
          </MDBCard>
        </MDBCol>
      </MDBRow>
    </div>

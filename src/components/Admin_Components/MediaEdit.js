...
import fetchMedia from "../Advertiser_Components/Fetch/fetchMedia";
import React, { useState } from "react";
import { useQuery } from "@tanstack/react-query";
import { ToastContainer, toast } from "react-toastify";
import axios from "axios";
const MediaEdit = (props) => {
  var id_arda=props.id_arda;
  ...
  const result = useQuery(["media", id_arda.id, token], fetchMedia);
  if (result.isLoading) {
    return (
      <MDBSpinner
  ...
  const handleEdit = (e) => {
    if (
      e.target.value === user.data[0].twitter &&
      e.target.value === user.data[0].instagram &&
      e.target.value === user.data[0].youtube &&
      e.target.value === user.data[0].tiktok
    ) {
  ...
  const pushEditUser = async () => {
    axios
      .put(
        `https://tata-backend.onrender.com/api/updateMediaLinks/${id_arda.id}`,
        newData,
        {
          headers,
        }
